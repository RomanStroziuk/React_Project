...
const SneakerWarehouseComponent = () => {
  const [newSneakerWarehouse, setNewSneakerWarehouse] = useState(null);
  const [searchTerm, setSearchTerm] = useState(""); 
  const [openSnackbar, setOpenSnackbar] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");
  const [sneakerQuantityMin, setSneakerQuantityMin] = useState(0); 
  const [sneakerQuantityMax, setSneakerQuantityMax] = useState(1000); 
  ...
  const filteredSneakerWarehouse = sneakerWarehouses.filter(
    (sneakerWarehouse) =>
      (sneakerWarehouse.sneakerId
        ...
        sneakerWarehouse.warehouseId
          .toLowerCase()
          .includes(searchTerm.toLowerCase())) &&
      sneakerWarehouse.sneakerQuantity >= sneakerQuantityMin && 
      sneakerWarehouse.sneakerQuantity <= sneakerQuantityMax 
  );
  ...
  return (
    <>
      ...
      <SearchBar
        ...
        placeholder="Search Sneaker Warehouses..."
      />
      ...
      <div>
        <input
          type="number"
          value={sneakerQuantityMin}
          onChange={(e) => setSneakerQuantityMin(Number(e.target.value))}
          placeholder="Min Quantity"
        />
        <input
          type="number"
          value={sneakerQuantityMax}
          onChange={(e) => setSneakerQuantityMax(Number(e.target.value))}
...
export default SneakerWarehouseComponent;
